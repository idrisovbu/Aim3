##----------------------------------------------------------------
##' Title: B_expenditure_decomp_with_cases.R
##'
##' Purpose: 
##----------------------------------------------------------------
...
fp_output <- file.path(dir_output, "spend_mean_decomp.csv")
...
d <- d %>%
  select(c("cause_id", "acause", "cause_name", "location_id", "location_name", "sex_id", "year_id", 
           "prevalence", "population", "spend_mean", "age_name", "age_group_years_start"
  ))

# Rename spend_mean -> expenditure
d <- d %>%
  setnames(old = "spend_mean", new = "expenditure") %>%
...
d_t[, "delta_expenditure" := get(paste0("expenditure_", end_year)) - get(paste0("expenditure_", start_year))]
