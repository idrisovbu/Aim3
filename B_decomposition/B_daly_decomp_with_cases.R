...
parser$add_argument("--input_file", help="Input data file with values of population, case counts, and DALYs for each cause, age group, sex, year, and draw.",
                    type="character")
...
d <- dcast(d, draw + cause_id + age_group_id + sex_id ~ year_id,
           value.var = c(factor_names, "burden"))
d <- decompose(d, factor_names=factor_names, start_year=start_year, end_year=end_year)
d[, "delta_dalys" := get(paste0("burden_", end_year)) - get(paste0("burden_", start_year))]
